...
def processor(path, max_chars):
    ...
    combined_data = data.groupby(pd.Grouper(freq=f'30s')).agg({'Device Name': 'first', 'Transcript': lambda x: ' '.join(x)})
...
def raw_summary(text, gensim_ratio):
    ...
    summary = summarize(text,gensim_ratio) # generates a summary of the text using the summarization function with a ratio of 0.05
    return summary

def final_summary(text, gpt_tokens):
    ...
    response = openai.Completion.create(
      ...
      prompt=prompt,
      max_tokens=gpt_tokens,  # sets the maximum number of tokens for the generated summary
      n=1,
      stop=None,
...
def summary_generator(processed_df, gensim_ratio, gpt_tokens):
    ...
    for index, row in processed_df.iterrows():
        ...
        summary = raw_summary(text, gensim_ratio)
        output_summary = final_summary(summary, gpt_tokens)
...
processed_df = processor('focusgroupday2.csv', max_chars)
print(processed_df['Start Time'])
df = summary_generator(processed_df, gensim_ratio, gpt_tokens)
